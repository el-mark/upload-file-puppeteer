...
(async () => {
	...
	await page.goto('https://es.imgbb.com/');
	...
	await page.waitForSelector('.btn.btn-big.green');
	await page.evaluate(() => document.getElementsByClassName('btn btn-big green')[0].click());

	// wait for selector that contains the uploaded file URL
	await page.waitForSelector('#uploaded-embed-code-0');
	await page.waitFor(5000);

	// get the download URL
	let downloadUrl = await page.evaluate(() => {
		return document.getElementById('uploaded-embed-code-0').value;
	});
